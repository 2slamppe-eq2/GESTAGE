...
<form method="post" action=".?controleur=administrateur&action=validationcreerutilisateur&id=<?php echo $this->id; ?>" name="CreateUser">
  <h1>Création d'utilisateur</h1>
    <!-- Choix du type de compte pour affiché les diférente information pour crée un compte spécifique -->
        ...
        <div id="Formulaire_Maitre_Stage" style="visibility:hidden;height:0;">
        <fieldset>
        <legend>Information spécifique aux Maitre de Stage</legend>     
        <label for="organisation">Organisation :</label>
        <select name ="organisation" id="organisation">
            <option value="--"></option>
            <?  //création du contenue du select pour les organisations
            $tab1=array();
            $cpt1=0;
            foreach ($this->lesEntreprises as $data) {   
              
            $tab1[$cpt]=$data->IDORGANISATION;// on stock les valeurs dans un tableau 
            echo'<option value="'.$tab1[$cpt].'">'.$data->NOM_ORGANISATION.'</option>'; //echo de la ligne 
            $cpt1=$cpt1+1;
            
        }
        ?>
        </select>
        <label for="formation">Fonction :</label>
    ...
    <!-- Donnée de conexion des utilisateurs -->
    <fieldset>
